...
$S$: \wss{What is the type of the state variable?}seq of T
...
\noindent pop():
\begin{itemize}
\item output: \wss{What should go here?}$out := \text{new Stack}(S[0..|s| - 2])$

\item exception: \wss{What should go here?}$exc := (\neg (|s| > 0)) \Rightarrow out\_of\_range)$
...
\noindent size():
\begin{itemize}
\item output: \wss{What should go here?}$out := |s|$
\item exception: None
\end{itemize}
...
\item output: \wss{What goes here?  The deck moves involves moving a card from
    the deck stack to the waste stack.}$D.size() > 0$
\item exception: None
\end{itemize}
...
\begin{tabular}{|p{3cm}|l|}
\hhline{|-|-|}
$c = \mbox{Tableau}$ & W, T[$n$] := \wss{What goes here?}W.pop(),T[$n$].push(W.top())\\
\hhline{|-|-|}
$c = \mbox{Foundation}$ & W, F[$n$] := \wss{What goes here?}W.pop(),F[$n$].push(W.top())\\
\hhline{|-|-|}
\end{tabular}
...
\noindent deck\_mv():
\begin{itemize}
\item transition: D, W := \wss{What goes here?}D.pop(),W.push(D.top())
\item exception:
...
\begin{itemize}
\item output: \wss{What goes here?}$\text{cnt\_cards}(T, F, D, W, f) \equiv 104$
\item exception: None
...
\noindent $\text{cnt\_cards\_stack}(s, f) \equiv (s.size())$\\
...
\noindent valid\_tab\_foundation($n_0, n_1$) $\equiv$ \wss{What goes here?  You
  ...
  \begin{tabular}{|p{4cm}|p{3.5cm}|l|}
  \hhline{|-|-|-|}
  $T[n_0].\text{size}() > 0$ & F[$n_1$].size() $>$ 0 & tab\_placeable(T[$n_0$].top(), F[$n_1$].top())\\
  \hhline{|~|-|-|}
   & F[$n_1$].size() = 0 & True\\
  \hhline{|-|-|-|}
...
\noindent tab\_placeable($n_0, n_1$) $\equiv$ $(n_0.top().s \equiv n_1.top().s \land n_0.top().r < n_1.top().r)$
